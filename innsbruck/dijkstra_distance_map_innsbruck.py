...
def generate_paths(df, graph):
    ...
    for lo, la in zip(df["long"], df["lat"]):
        target_points.append((la, lo))

    # origin = (df['lat'][15], df['long'][15])

    for index, row in df.iterrows():

        # Setup the origin point
        origin = (row['lat'], row['long'])
...
def plot_map(df, long, lat):
    ...
    for index, row in df.iterrows():
        fig.add_trace(go.Scattermapbox(
            ...
            lon=df["long"],
            lat=df["lat"],
            marker={"size": 12, "color": 'rgba(3,3,3,0.5)', },
            opacity=0.1))
    ...
    fig.write_image(OS_PATH + '/innsbruck/innsbruck_dijkstra_map.jpg', scale=3)
...
SENSORS_CSV = os.path.join(OS_PATH, 'innsbruck/innsbruck_sensors.csv')
