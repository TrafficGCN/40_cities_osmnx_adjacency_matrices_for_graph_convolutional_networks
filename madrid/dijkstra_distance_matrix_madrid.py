...
def generate_adjacency_matrix(df, df_row, row_index, graph):
    ...
    np.savetxt(OS_PATH + "/madrid_adjacency_matrix_" + str(row_index) + ".csv", matrix, delimiter=",", fmt='%s')
...
def combine_csv_files(df):
    ...
    for i in range(1, len(df) + 1):
        file_path = OS_PATH + "/madrid_adjacency_matrix_" + str(i) + ".csv"
    ...
    result.to_csv(OS_PATH + "/madrid_adjacency_matrix.csv", index=False)
...
if __name__ == '__main__':
    # Data import path
    OS_PATH = os.path.dirname(os.path.realpath('__file__'))
    SENSORS_CSV = os.path.join(OS_PATH, 'madrid_sensors.csv')
    ...
    # 'drive', 'bike', 'walk'
    graph = create_graph("Madrid, Spain", "drive")
    ...
    adjacency_matrix_file = os.path.join(OS_PATH, 'madrid_adjacency_matrix.csv')
    transpose_adjacency_matrix(adjacency_matrix_file)

    transposed_matrix_file = os.path.join(OS_PATH, 'madrid_adjacency_matrix_transposed.csv')
