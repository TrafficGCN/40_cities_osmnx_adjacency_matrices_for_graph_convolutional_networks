...
def generate_adjacency_matrix(df, df_row, row_index, graph):
    ...
    np.savetxt(OS_PATH + "/los_angeles_adjacency_matrix_" + str(row_index) + ".csv", matrix, delimiter=",", fmt='%s')
...
def combine_csv_files(df):
    ...
    for i in range(1, len(df) + 1):
        file_path = OS_PATH + "/los_angeles_adjacency_matrix_" + str(i) + ".csv"
    ...
    result.to_csv(OS_PATH + "/los_angeles_adjacency_matrix.csv", index=False)
...
if __name__ == '__main__':
    # Data import path
    OS_PATH = os.path.dirname(os.path.realpath('__file__'))
    SENSORS_CSV = os.path.join(OS_PATH, 'los_angeles_sensors.csv')
    ...
    # Divide the dataframe into four sets of roughly equal size
    num_processes = min(400, len(df))
    df_chunks_1 = divide_dataframe(df[:100], num_processes//4)
    df_chunks_2 = divide_dataframe(df[100:200], num_processes//4)
    df_chunks_3 = divide_dataframe(df[200:300], num_processes//4)
    df_chunks_4 = divide_dataframe(df[300:], num_processes//4)

    # Create the networkx graph
    # 'drive', 'bike', 'walk'
    graph = create_graph("Los Angeles, California, United States", "drive")

    # Create processes for the first set of chunks
    processes_1 = []
    for i, df_chunk in enumerate(df_chunks_1):
    ...
    # Create processes for the second set of chunks
    processes_2 = []
    for i, df_chunk in enumerate(df_chunks_2):
        p = mp.Process(target=generate_adjacency_matrix, args=(df, df_chunk, i+len(df_chunks_1)+1, graph))
        p.start()
    ...
    # Create processes for the third set of chunks
    processes_3 = []
    for i, df_chunk in enumerate(df_chunks_3):
        p = mp.Process(target=generate_adjacency_matrix, args=(df, df_chunk, i+len(df_chunks_1)+len(df_chunks_2)+1, graph))
        p.start()
        processes_3.append(p)

    for p in processes_3:
        p.join()

    # Create processes for the fourth set of chunks
    processes_4 = []
    for i, df_chunk in enumerate(df_chunks_4):
        p = mp.Process(target=generate_adjacency_matrix, args=(df, df_chunk, i+len(df_chunks_1)+len(df_chunks_2)+len(df_chunks_3)+1, graph))
        p.start()
        processes_4.append(p)

    for p in processes_4:
        p.join()

    # Combine the CSV files
    combine_csv_files(df_chunks_1 + df_chunks_2 + df_chunks_3 + df_chunks_4)

    # Transpose the combined adjacency matrix and save it as a CSV file
    adjacency_matrix_file = os.path.join(OS_PATH, 'los_angeles_adjacency_matrix.csv')
    transpose_adjacency_matrix(adjacency_matrix_file)

    transposed_matrix_file = os.path.join(OS_PATH, 'los_angeles_adjacency_matrix_transposed.csv')
