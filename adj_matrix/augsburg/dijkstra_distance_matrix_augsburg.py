...
def generate_adjacency_matrix(df, df_row, row_index, graph):
    ...
    np.savetxt(OS_PATH + "/augsburg_adjacency_matrix_" + str(row_index) + ".csv", matrix, delimiter=",", fmt='%s')
...
def combine_csv_files(df):
    ...
    for i in range(1, len(df) + 1):
        file_path = OS_PATH + "/augsburg_adjacency_matrix_" + str(i) + ".csv"
    ...
    result.to_csv(OS_PATH + "/augsburg_adjacency_matrix.csv", index=False)
...
if __name__ == '__main__':
    ...
    # 'drive', 'bike', 'walk'
    graph = create_graph("Augsburg, Bavaria, Germany", "drive")
    ...
    adjacency_matrix_file = os.path.join(OS_PATH, 'augsburg_adjacency_matrix.csv')
    transpose_adjacency_matrix(adjacency_matrix_file)

    transposed_matrix_file = os.path.join(OS_PATH, 'augsburg_adjacency_matrix_transposed.csv')
